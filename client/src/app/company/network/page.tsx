...
export default function CompanyNetwork() {
  ...
  const loadData = async () => {
    try {
        ...
        const [collegesRes, requestsRes] = await Promise.all([
            axios.get("http://localhost:5000/api/network/search-colleges"),
            axios.get(`http://localhost:5000/api/network/requests/${userId}`)
        ]);
  ...
  const handleConnect = async (collegeId: string) => {
    try {
        ...
        await axios.post("http://localhost:5000/api/network/connect", {
            requesterId,

...
export default function CompanyRequests() {
  ...
  const fetchRequests = async () => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:5000/api/network/requests/${userId}`);
        setRequests(data);
        setLoading(false);
    } catch (error) {
  ...
  const handleRespond = async (id: string, status: 'Active' | 'Rejected') => {
    try {
        await axios.put("http://localhost:5000/api/network/respond", { partnershipId: id, status });

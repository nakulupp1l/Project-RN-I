...
export default function Settings() {
    ...
    const loadData = async () => {
        ...
        if(storedUser) {
            ...
            if(user.role === 'college' || user.role === 'college_member') {
                ...
                try {
                    const res = await axios.get(`http://localhost:5000/api/auth/team/${collegeId}`);
                    setTeam(res.data);
                } catch(e) {
  ...
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const res = await axios.post("http://localhost:5000/api/auth/add-staff", { ...newStaff, collegeId });
  ...
  return (
    <CollegeLayout>
      <div className="p-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">Settings ⚙️</h1>

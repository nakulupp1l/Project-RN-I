...
export default function StudentDirectory() {
  ...
  const fetchStudents = async () => {
    try {
        ...
        const collegeId = JSON.parse(storedUser)._id;

        const { data } = await axios.get(`http://localhost:5000/api/auth/students/${collegeId}`);
        setStudents(data);
        setLoading(false);
    } catch (error) {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    try {
        ...
        await axios.post("http://localhost:5000/api/auth/add-student", { ...formData, collegeId });
        alert(`Student added!`);
  ...
  const processBulkUpload = async () => {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            const res = await axios.post("http://localhost:5000/api/auth/add-students-bulk", { students: jsonData, collegeId });
